...
const GamePage = (props) => {
  ...
  useEffect(() => {
    ...
    if (playerData?.playWithTime && playerData?.increment != 0) {
      if (isBotTurn) {
        setBotTime(
          (prevTime) => prevTime + parseInt(playerData?.increment || "0")
        );
      } else {
        setPlayerTime(
          (prevTime) => prevTime + parseInt(playerData?.increment || "0")
  ...
  const makeBotMove = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/stockfish/api/chess/best-move",
        {
          fen: fen,
        }
      );
  ...
  const handleColorChange = (event) => {
    if (gameInProgress) {
      toast.error(
        "You cannot change color during a game. Please end the current game first."
      );
      return;
    }
    ...
    <div className="flex justify-center items-center flex-col">
      ...
      <div className="mb-4 flex gap-4">
        ...
        <button
          ...
          disabled={
            (!isReviewMode && !gameInProgress) || playerData?.playWithTime
          } // Disable if not in review mode
          className="px-4 py-2 bg-blue-500 text-white rounded disabled:bg-gray-400"
        >
          Resume Game
        </button>
      ...
      <div className="flex w-[100%] gap-10 mb-4">
        <div className="flex flex-col gap-4 text-sm flex-1">
          <span>
            Hello, {playerData?.name || "Player"}! You are playing as{" "}
            {playerData?.player || playerColor}.
          </span>
        ...
        >
          ...
          <div className="absolute bottom-4 left-2 right-2 flex flex-col justify-between">
            ...
            <div className="bg-gray-900 rounded-sm py-2 px-4 flex justify-between items-center">
      ...
      <div className="flex gap-4">
        ...
        <div>
          <label htmlFor="colorSelect" className="mr-2">
            Choose Your Color:
          </label>
          <select
            id="colorSelect"
        ...
        <div className="mb-4">
          <label htmlFor="showMoves" className="mr-2 text-white">
            Show valid moves:
          </label>
